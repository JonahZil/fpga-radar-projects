...
def receiver_thread(ser):
    ...
    while receiver_running:
        ...
        if previous_frame_time is None:
            print(
                f"Frame {frame_number} received | "
                f"invalid24={invalid_24bit}"
            )
        else:
            frame_interval_ms = (
                current_frame_time -
        ...
        if invalid_24bit:
            ...
            print(
                f"Corrupted frame: {invalid_count} values "
                f"outside signed 24-bit range"
            )
